  ...
  <div class="container">
    ...
    <div class="form-box">
      ...
      <radio-group @change="onIdentityChange" class="radio-group">
        <label class="radio-item"><radio value="student" checked="true" />留学生</label>
        <label class="radio-item"><radio value="tourist" />游客</label>
        <label class="radio-item"><radio value="worker" />在日工作</label>
      </radio-group>
...
.container { padding: 40px 20px; background-color: #fff; min-height: 100vh; }
.header { margin-bottom: 30px; margin-top: 40px; }
.title { font-size: 30px; font-weight: bold; margin-bottom: 10px; }
.subtitle { color: #999; }
.input {
  width: 100%; height: 50px; background: #f5f5f5; border-radius: 8px;
  margin-bottom: 15px; padding: 0 15px; box-sizing: border-box;
}
.label { margin: 10px 0 5px; color: #666; font-size: 14px; }
.radio-group { display: flex; justify-content: space-between; margin-bottom: 20px; }
.radio-item { font-size: 14px; }
.btn-submit {
  width: 100%; height: 50px; background: #4cd964; color: #fff; /* 注册按钮绿色 */
  border-radius: 25px; line-height: 50px; font-size: 18px; margin-top: 10px;
}
.link-area { margin-top: 20px; text-align: center; color: #007aff; }
</style>